...
$params = @{
    ...
    resolutionState             = if($alert.ResolutionState -eq 0) {"New"} elseif($alert.ResolutionState -eq 255) {"Closed"} elseif($alert.ResolutionState -eq 254) {"Resolved"} elseif($alert.ResolutionState) {$alert.ResolutionState} else {"Not Present"}
